...
Stand 2026-08-12 — Erweiterung: topics bis 8 inkl. Personen, agentName.
Aktiv in der Scheduled Task **fc_news_agent** (vorher fc_geo_news_locating).
Diesen Text 1:1 in die Claude-Scheduled-Task übernehmen (der Auftragstext
liegt NICHT im Repo; Felder, die er nicht aufzählt, füllt der Agent nicht).
Änderungen gegenüber der vorherigen Fassung sind mit ⬅ NEU markiert — die
Marker beim Übernehmen entfernen.
...
1. get_news_for_geocoding {"limit": 20, "from": "<Datum vor 2 Tagen, YYYY-MM-DD>"}
   → Tabelle newsId|date|category|source|title|description. Das from-Fenster
   ist Absicht — der historische Backlog ohne Verortung wird NICHT abgearbeitet.
2. Bestimme je Zeile: lat/lon (WGS84, relevantester Ort), country (ISO alpha-2),
   ...
   Schlagzeile, max 90 Zeichen), topics (1–8 Themen-Tags, typisch 2–5). ⬅ NEU: bis 8
   topics: konkrete Themen der News, Englisch, Title Case — z.B.
     ...
     Personen mit vollem Namen, wenn sie das Ereignis prägen — z.B.
     "Zohran Mamdani", "Jerome Powell", "Elon Musk". ⬅ NEU: Personen-Tags
     ...
     dasselbe Thema und dieselbe Person deshalb IMMER mit exakt demselben Tag,
   ...
   "topics":[...], "headline":"...", "summary":"..."} — relevance, topics,
   headline und summary werden auch OHNE Ort angegeben (Themen und
   Wichtigkeit sind ortsunabhängig; Marktberichte sind der Hauptfall).
   Nur lat/lon/country/place/precision/confidence entfallen.
3. submit_news_locations {"agentName":"fcNewsAgent","items":[...]} — alle 20
   in EINEM Aufruf; agentName immer mitsenden, er wird als enrichedBy
   gespeichert. ⬅ NEU: agentName
   Bei Verortung sind lat, lon, country, precision und relevance Pflicht.
   ERROR-Zeilen einmalig korrigieren und nachreichen. Validierungsfehler
   auf Schema-Ebene (z.B. mehr als 8 topics, ein topic unter 2 Zeichen) lehnen
...
relevance-Wert und ihrer headline, und die 5 häufigsten topics des Laufs.
